...
# 2. 환경 변수(전역 변수) -> .env, airflow 환경 변수
BUCKET_NAME      = "de-ai-16-infra-s3-bk-827913617635"
UPLOAD_FILE_NAME = "sensor_data.csv"              # 현재 시간 등 인식/구분 정보 누락
S3_KEY           = f'airflow/{UPLOAD_FILE_NAME}'  # s3 상 위치 조정
...
with DAG(  
  ...
  task_upload_to_s3 = LocalFilesystemToS3Operator(
    ...
    dest_key  = S3_KEY,  # s3 버킷 내에서 객체간 구분하는 키 -> 파일명 대체
